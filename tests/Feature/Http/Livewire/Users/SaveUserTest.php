...
class SaveUserTest extends TestCase
{
    ...
    public function render_for_edit()
    {
        ...
        Livewire::actingAs($this->admin)
            ...
            ->assertSet('role_id', $user->role->id)
    ...
    public function store_new_user()
    {
        Livewire::actingAs($this->admin)
            ->test(SaveUser::class)
            ->set('role_id', $this->admin->role->id)
    ...
    /** @test */
    public function see_existing_user_data()
    {
        $user = factory(User::class)->create([
            'email' => 'jane@example.com',
            'role_id' => factory(Role::class)->create(),
        ...
        Livewire::actingAs($this->admin)
            ->test(SaveUser::class, ['user' => $user])
            ->assertSet('email', $user->email)
            ->assertSet('role_id', $user->role_id);
    }
    ...
    public function clientFormValidationProvider()
    {
        return [
            ...
            'Test role_id is required' => ['role_id', '', 'required'],
            'Test role_id must exist' => ['role_id', 'invalid-role-id', 'exists'],
        ];
    }
